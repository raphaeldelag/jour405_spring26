...
The strongest correlates of turnout are median_age (positive) and pct_white (positive), and both hold up in the scatterplots with reasonably clean linear trends. Income and pct_bachelors_higher both track positively with turnout, but they also track tightly with each other, which is a real problem for any model trying to isolate either one's effect. pct_unemployed runs negative with turnout, though the relationship is noisy. Population is the weakest of the bunch, with the scatterplot basically flat until you hit the big urban outliers like Baltimore City and Prince George's County.

The plots I'd trust most going into a model are median_age and pct_white. The ones I'd be skeptical of are income and pct_bachelors_higher together, because they're measuring overlapping things.
...
With six predictors and only 24 observations, I expected a lot of noise, and that's what the model delivers. Several variables are not statistically significant, and income and pct_bachelors_higher likely absorb each other's variance because they measure such similar things. The adjusted R-squared tells a more honest story than the raw R-squared here, and with this many predictors crammed into 24 counties, it will be noticeably lower. The coefficients for median_age and pct_white are probably the most reliable; the others I'd treat with skepticism.

It's not a useless model, but I wouldn't publish a story off it as-is. The sample is too small and too many variables are tangled up with each other to isolate anything cleanly. I'd use this as a starting point to figure out which two or three variables are actually doing the work.
...
Model 2 drops income to deal with the overlap between income and education — they were measuring too similar a thing. Model 3 goes further and keeps only the three variables that had the cleanest scatterplot relationships: median_age, pct_white, and pct_unemployed. Model 4 swaps pct_white out for pct_bachelors_higher to test whether education predicts better than race once age and unemployment are already in.

If Model 3's adjusted R-squared comes out near the full model's, that's a cleaner, more usable result than six variables that undercut each other. A simpler model I can actually explain is more useful for a story than a technically richer one I can't. My first call after this would be to a Maryland political scientist to ask whether the age relationship reflects who lives in these counties or how different age cohorts actually turn out.
