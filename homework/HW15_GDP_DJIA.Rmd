...
```{r}
economy <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/gdp_djia.csv")
```
...
```{r}
economy |> summarize(correlation = cor(DJIA, GDP, method="pearson"))
```
...
The correlation between GDP and the DJIA is 0.953, which is about as strong as it gets. The two indicators moved almost in lockstep over the past decade which we can clearly see in the scatterplot. The points follow the regression line tightly from lower-left to upper-right, with a noticeable dip during the COVID-era quarters before the trend picks back up. Both variables likely respond to the same underlying economic conditions rather than one driving the other.
