---
title: "Sampling"
---

Follow along with the demo to learn how to sample data from a population using R and the Tidyverse. You should get started by loading the tidyverse in a new R block.

### Task 1: Load the tidyverse

```{r}
library(tidyverse)
```

### Task 2: Load 1,000 rows of data with the ages of people from 18-80
```{r}
set.seed(42)

population <- tibble(
  age = sample(18:80, 1000, replace = TRUE)
)
```

### Task 3: Sample 100 rows of data from the dataset
```{r}
sampled_data <- population %>%
  slice_sample(n = 100)

sampled_data
```


### Task 4: Calculate the mean age and standard deviation of the sample
```{r}
sample_stats <- sampled_data %>% 
  summarise(
    mean_age = mean(age),
    sd_age = sd(age)
  )

sample_stats
```

### Task 5: Calculate the mean age and standard deviation of the population
```{r}
population_stats <- population %>% 
  summarise(
    mean_age = mean(age),
    sd_age = sd(age)
  )

population_stats
```

### Task 6: Compare the sample and population statistics

Compare the means and standard deviations. Are they similar? Go back and re-run the sampling and statistics calculations with a smaller sample size. How do the results change?

```{r}
sampled_data_1000 <- population %>%
  slice_sample(n = 1000, replace = TRUE)

sample_stats_1000 <- sampled_data_1000 %>%
  summarise(
    mean_age = mean(age),
    sd_age = sd(age)
  )

sample_stats_1000
```

```{r}
sampled_data_10 <- population %>%
  slice_sample(n = 10)

sample_stats_10 <- sampled_data_10 %>%
  summarise(
    mean_age = mean(age),
    sd_age = sd(age)
  )

sample_stats_10
```

I decided to run the analysis at three different sample sizes: 10, 100 and 1,000. At n=1000, the sample mean was 50.1 with an SD of 17.7—nearly identical to the population (49.9, 17.5). At n=100, the mean came in at 48.8 with an SD of 18.9, showing a bit more drift. When I reduced the sample size to just a handful of people at n=10, the mean shifted to 51.4 and the SD jumped to 21.3. The pattern is consistent: the larger the sample, the more it mirrors the population. A smaller slice of the data is just more at the mercy of random variation. In other words, the smaller the sample, the wider the swings.


When finished, save your work, switch to GitHub Desktop, then add, commit and push your changes to GitHub and submit the URL of the notebook in ELMS.
